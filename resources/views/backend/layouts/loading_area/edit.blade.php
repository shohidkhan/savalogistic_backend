            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-body">
                        ...
                        <form action="{{ route('admin.loading_area.update',$data->id) }}" method="POST" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                             <div class="row">
                                 <div class="col-lg-4 mt-4">
                                    ...
                                    <select name="loading_zone_id"  class="form-control @error('loading_zone_id') is-invalid @enderror">
                                        ...
                                        @foreach ($zones as $zone)
                                            <option value="{{ $zone->id }}" {{ old('loading_zone_id',$data->loading_zone_id) == $zone->id ? 'selected': '' }}>{{ $zone->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('country_id')
